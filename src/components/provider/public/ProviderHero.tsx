...
export function ProviderHero({
  ...
  // Single source of truth for live availability (server-resolved).
  const { status: liveStatus } = useProviderLiveStatus({ slug: profile.provider_slug });
  ...
  return (
    ...
    >
      <div className="grid gap-0 sm:grid-cols-[minmax(0,220px)_1fr] lg:grid-cols-[minmax(0,240px)_1fr] xl:grid-cols-[minmax(0,360px)_1fr]">
        ...
        <div className="flex flex-col gap-2.5 p-5 sm:p-6 lg:gap-3 lg:p-8 xl:justify-center xl:p-10">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {liveStatus ? (
              <span data-testid="provider-availability-status">
                <ProviderStatusPill status={liveStatus} showMessage showPresence useLongLabel />
              </span>
            ) : (
              <p

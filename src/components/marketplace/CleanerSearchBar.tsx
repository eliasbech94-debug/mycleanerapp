...
export function CleanerSearchBar({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <form
      onSubmit={submit}
      className={`grid w-full grid-cols-1 gap-px rounded-2xl border border-[hsl(var(--mkt-border))] bg-[hsl(var(--mkt-surface))] p-1 shadow-[var(--mkt-shadow-lift)] sm:grid-cols-[1.6fr_1fr_1.2fr_auto] ${compact ? "" : ""}`}
    ...
    >
      <Field label={t("search.where", "Where")} icon={<MapPin className="h-4 w-4" />}>
        <input
          ...
          className="w-full bg-transparent text-[15px] text-[hsl(var(--mkt-ink))] placeholder:text-[hsl(var(--mkt-ink-soft))] focus:outline-none"
      ...
      <Field label={t("search.when", "When")} icon={<CalendarIcon className="h-4 w-4" />}>
        <input
          ...
          className="w-full bg-transparent text-[15px] text-[hsl(var(--mkt-ink))] focus:outline-none"
...
function Field({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <label className="group flex flex-col gap-0.5 rounded-xl px-4 py-3 transition hover:bg-[hsl(var(--mkt-surface-muted))] focus-within:bg-[hsl(var(--mkt-surface-muted))]">
      <span className="text-[11px] font-semibold uppercase tracking-wider text-[hsl(var(--mkt-ink-soft))]">{label}</span>
      <span className="flex items-center gap-2 text-[hsl(var(--mkt-ink-muted))]">
        {icon}
        {children}
      </span>
    </label>

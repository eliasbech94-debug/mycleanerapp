...
export function CleanerResultsList({
  ...
  const shown = useMemo(() => (providers ?? []).slice(0, 4), [providers]);
  // One batched status lookup for the whole rail.
  const { get: getStatus } = useProviderLiveStatuses({
    slugs: shown.map((p) => p.provider_slug),
    enabled: !isDemo,
  });
          ...
          <ul className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            {shown.map((p) => (
              <li key={p.provider_slug}>
                <CompactProviderCard provider={p} liveStatus={getStatus(p.provider_slug)} />
              </li>
            ))}
          </ul>
        )}

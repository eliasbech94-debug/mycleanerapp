...
export function RoleChoiceDialog({ open, onOpenChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="w-[min(100vw-1.5rem,34rem)] max-w-none border-white/30 bg-background/85 shadow-2xl backdrop-blur-xl sm:w-[min(100vw-3rem,34rem)]"
      ...
      >
        ...
        <div className="mt-2 grid gap-3 sm:grid-cols-2 sm:gap-2.5 lg:gap-3">
          <button
            type="button"
            onClick={() => go(CUSTOMER_SIGNUP)}
            className="group flex flex-col items-start gap-3 rounded-2xl border border-[hsl(var(--mkt-border))] bg-[hsl(var(--mkt-surface))] p-4 text-left sm:p-5 min-h-[112px] transition hover:border-[hsl(var(--mkt-brand))] hover:shadow-[var(--mkt-shadow-lift)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--mkt-brand))]"
          >

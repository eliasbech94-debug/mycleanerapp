    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-serif">Operationsoversigt</h1>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Kpi label="Notifikations-kø" value={data?.counters.notification_backlog ?? 0} threshold={100} />
        <Kpi label="Åbne tvister" value={data?.counters.open_disputes ?? 0} threshold={5} />
        <Kpi label="GDPR-jobs aktive" value={data?.counters.gdpr_jobs_active ?? 0} threshold={10} />
        <Kpi label="Regnskabs-mismatch" value={data?.counters.reconciliation_alerts_open ?? 0} threshold={1} />
      </div>

      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><ShieldAlert className="h-5 w-5" /> Åbne alarmer</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {(data?.alerts ?? []).length === 0 && <p className="text-sm text-muted-foreground">Ingen åbne alarmer.</p>}
          {(data?.alerts ?? []).map(a => (
            <div key={a.id} className="flex items-center justify-between border rounded p-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {a.source} · sidst set {new Date(a.last_seen_at).toLocaleString("da-DK")} · {a.seen_count}×
                </div>
                {a.body && <div className="text-sm mt-1">{a.body}</div>}
              </div>
              <Button size="sm" variant="outline" onClick={() => resolveAlert(a.alert_key)}>Løs</Button>
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle>Fejl seneste 24 t</CardTitle></CardHeader>
          <CardContent className="text-sm space-y-1">
            ...
            {Object.keys(errorsByCategory).length === 0 && <p className="text-muted-foreground">Ingen fejl.</p>}
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Webhook-fejl (24 t)</CardTitle></CardHeader>
          <CardContent className="text-sm space-y-1">
            ...
            {webhookFail.length === 0 && <p className="text-muted-foreground">Ingen fejl.</p>}
          </CardContent>
        </Card>
      </div>

      <Card>
        <CardHeader><CardTitle>Baggrundsjobs (24 t)</CardTitle></CardHeader>
        <CardContent className="text-sm space-y-1">
          {jobFailures.slice(0,10).map((j,i) => (
            <div key={i} className="flex justify-between border-b py-1">
              <span>{j.job_name}</span>
              <span className="text-muted-foreground">{j.duration_ms ?? "–"} ms · {j.failed_count} fejl</span>
            </div>
          ))}
          {jobFailures.length === 0 && <p className="text-muted-foreground">Alle jobs kørte uden fejl.</p>}
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle>Åbne incidents</CardTitle></CardHeader>
        <CardContent className="text-sm space-y-1">
          ...
          {(data?.incidents ?? []).length === 0 && <p className="text-muted-foreground">Ingen åbne incidents.</p>}
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle>Seneste deployments</CardTitle></CardHeader>
        <CardContent className="text-sm space-y-1">
          ...
          {(data?.deployments ?? []).length === 0 && <p className="text-muted-foreground">Ingen deployments registreret.</p>}
        </CardContent>
      </Card>
    </div>
  );
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <Badge variant={sevColor(level)}>{level}</Badge>
        </div>
      </CardContent>
    </Card>
  );

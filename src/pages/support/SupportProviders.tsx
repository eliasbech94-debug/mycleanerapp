...
export default function SupportProvidersPage() {
  ...
  return (
    <SupportLayout title="Providere" description="Support-sikker provideroversigt. Ingen bank- eller skattedata.">
      <form
      ...
      >
        <div className="relative flex-1">
          ...
          <Input
            ...
            placeholder="Søg navn, provider-id eller telefon…"
            className="pl-9"
            aria-label="Søg providere"
          />
        </div>
      ...
      {!isLoading && (data?.length ?? 0) === 0 && (
        <Card><CardContent className="p-10 text-center text-muted-foreground text-sm">Ingen providere fundet.</CardContent></Card>
      )}
              ...
              <CardContent className="p-4 flex flex-wrap items-center gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {p.country_code && <>Land {p.country_code} • </>}
                    Oprettet {new Date(p.created_at).toLocaleDateString("da-DK")}
                  </div>
                </div>
                {p.deactivated_at

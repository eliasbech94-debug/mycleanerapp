...
/**
 * Calls a Supabase RPC whose name is chosen at runtime.
 *
 * The generated client types require a literal function name, so the cast is
 * isolated here instead of being repeated as `any` at every call site.
 */
type RpcName = Parameters<typeof supabase.rpc>[0];
function callRpc(fn: string, args: Record<string, unknown>) {
  return supabase.rpc(fn as RpcName, args as never);
}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async () => {
                ...
                const { error } = await callRpc(fn, { _article_id: confirm.article.id });
                if (error) {
...
function ArticleEditor({ article, onClose, onChanged, canPublish, canArchive, onConfirm }: EditorProps) {
  ...
  async function call(fn: string, args: Record<string, unknown>, label: string) {
    setBusy(true);
    try {
      const { error } = await callRpc(fn, args);
            ...
            return (
              <div key={r.country_code} className="p-3 flex items-center gap-3 text-sm">
                ...
                <Button
                  ...
                  onClick={async () => {
                    setBusy(true);
                    const { error } = await callRpc("country_emergency_info_verify", { _country_code: r.country_code });
                    setBusy(false);
                ...
                {canPublish && (
                  <Button
                    ...
                    onClick={async () => {
                      setBusy(true);
                      const { error } = await callRpc("country_emergency_info_publish", { _country_code: r.country_code });
                      setBusy(false);

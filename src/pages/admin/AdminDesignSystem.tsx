...
export default function AdminDesignSystem() {
  ...
  return (
    <main className="container py-10 space-y-8">
      <header>
        <h1 className="font-heading text-2xl">{t("pages.adminDesignSystem.title")}</h1>
        <p className="text-sm text-muted-foreground">{t("pages.adminDesignSystem.subtitle")}</p>
      </header>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {tokens.map(([name, cls]) => (
            <div key={name} className={`rounded-lg p-4 ${cls}`}>
              ...
              <div className="text-sm">{t("pages.adminDesignSystem.sample")}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        <Card className="max-w-md">
          <CardHeader><CardTitle>{t("pages.adminDesignSystem.cardTitle")}</CardTitle></CardHeader>
          <CardContent className="text-sm text-muted-foreground">{t("pages.adminDesignSystem.cardBody")}</CardContent>
        </Card>
      </section>

      <section className="space-y-3">
        <h2 className="font-heading text-lg">Alerts</h2>
        <Alert><AlertDescription>{t("pages.adminDesignSystem.alertInfo")}</AlertDescription></Alert>
        <Alert variant="destructive"><AlertDescription>{t("pages.adminDesignSystem.alertError")}</AlertDescription></Alert>
      </section>

      <section className="space-y-2">
        <h2 className="font-heading text-lg">{t("pages.adminDesignSystem.catalogTitle")}</h2>

...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "signup" && !signupRole) {
      toast.error("Vælg om du vil oprette dig som kunde eller provider");
      return;
    }
    ...
    try {
      ...
      if (mode === "signup") {
        ...
        const role = signupRole as SignupRole;
        const { data, error } = await supabase.auth.signUp({
          email,
          password,
          options: {
            emailRedirectTo: callbackUrl,
            data: { full_name: fullName, country_code: country, signup_role: role },
        ...
        if (data.session) navigate(role === "provider" ? "/provider-onboarding" : "/customer", { replace: true });
        else navigate("/login?redirect=/auth/callback", { replace: true });
      } else if (mode === "forgot") {
  ...
  async function handleGoogle() {
    ...
    if (mode === "signup" && signupRole) {
      sessionStorage.setItem("pendingSignupRole", signupRole);
  ...
  return (
    <main className="min-h-screen font-editorial grid place-items-center px-4 py-8" style={{ background: C.cream, color: C.ink }}>
      <div className="w-full max-w-md">
        ...
        <div className="mt-6 rounded-3xl border-2 bg-white p-7 shadow-[8px_8px_0_rgba(10,61,58,0.15)]" style={{ borderColor: C.ink }}>
          ...
          <p className="mt-1 text-sm opacity-70">
            ...
            {mode === "signup" && "Vælg først, hvordan du vil bruge MyCleaner."}
            {mode === "forgot" && "Vi sender et gendannelseslink til din email."}
          </p>

          {mode === "signup" && (
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={() => setSignupRole("customer")}
                className={`rounded-2xl border-2 p-4 text-left transition ${signupRole === "customer" ? "shadow-[4px_4px_0_rgba(10,61,58,0.18)]" : "opacity-75 hover:opacity-100"}`}
                style={{ borderColor: signupRole === "customer" ? C.teal : `${C.ink}33`, background: signupRole === "customer" ? "#eefaf7" : "white" }}>
                <UserRound className="h-5 w-5" />
                <div className="mt-2 font-bold">Kunde</div>
                <div className="mt-1 text-xs opacity-70">Find og book en verificeret cleaner.</div>
              </button>
              <button type="button" onClick={() => setSignupRole("provider")}
                className={`rounded-2xl border-2 p-4 text-left transition ${signupRole === "provider" ? "shadow-[4px_4px_0_rgba(10,61,58,0.18)]" : "opacity-75 hover:opacity-100"}`}
                style={{ borderColor: signupRole === "provider" ? C.orange : `${C.ink}33`, background: signupRole === "provider" ? "#fff4ee" : "white" }}>
                <Sparkles className="h-5 w-5" />
                <div className="mt-2 font-bold">Provider / Cleaner</div>
                <div className="mt-1 text-xs opacity-70">Tilbyd rengøring og få nye kunder.</div>
              </button>
            </div>
          )}
          ...
          {mode !== "forgot" && (
            <>
              <button type="button" onClick={handleGoogle} disabled={loading || (mode === "signup" && !signupRole)}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            {mode === "signup" && (
              <div>
                ...
                <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)}
                  className="mt-1 w-full rounded-xl border-2 bg-white px-3 py-2.5 text-base focus:outline-none" style={{ borderColor: `${C.ink}33` }} />
              </div>
            )}
            <div>
              ...
              <>
                <div>
                  ...
                  <select value={country} onChange={(e) => setCountry(e.target.value)}
                    className="mt-1 w-full rounded-xl border-2 bg-white px-3 py-2.5 text-base focus:outline-none" style={{ borderColor: `${C.ink}33` }}>
                    <option value="DK">Danmark</option><option value="GB">United Kingdom</option><option value="SE">Sverige</option><option value="ES">Spanien</option>
                  </select>
                </div>
                <label className="flex items-start gap-2 rounded-xl border-2 p-3 text-sm" style={{ borderColor: `${C.ink}33` }}>
                  ...
                  <span>Jeg accepterer <Link to="/regler" target="_blank" className="underline">vilkårene</Link> og <Link to="/privatliv" target="_blank" className="underline">privatlivspolitikken</Link>.
                    {requiredDocs.length > 0 && <span className="ml-1 opacity-60">(version {requiredDocs.map((d) => `${d.kind}@${d.version}`).join(", ")})</span>}
                  </span>
                </label>
            ...
            <button type="submit"
              disabled={loading || !captchaToken || (mode === "signup" && (!signupRole || !acceptedLegal || legalStatus !== "ready"))}
              ...
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>
                {mode === "signin" && "Log ind"}{mode === "signup" && "Opret konto"}{mode === "forgot" && "Send gendannelseslink"}<ArrowRight className="h-4 w-4" />
              </>}
            </button>
          </form>

          {mode === "signin" && (
            <button type="button" onClick={() => setMode("forgot")} className="mt-3 w-full text-center text-xs opacity-70 hover:opacity-100 underline">Glemt adgangskode?</button>
          )}
          <button type="button" onClick={() => { setMode(mode === "signin" ? "signup" : "signin"); setSignupRole(null); }}

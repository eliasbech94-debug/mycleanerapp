...
/**
 * Shared field styling for the onboarding wizard.
 * Presentation only: 44px min tap height, calm borders and a visible focus ring.
 */
const INPUT_CLASS =
  "w-full min-h-[46px] rounded-xl border-2 border-[#0a3d3a2b] bg-white px-3.5 py-2.5 text-[15px] text-[#0a3d3a] placeholder:text-[#0a3d3a80] transition-colors focus:border-[#168a7a] focus:outline-none focus:ring-2 focus:ring-[#168a7a4d] disabled:opacity-60";
const CHIP_CLASS =
  "inline-flex min-h-[40px] items-center rounded-full border-2 px-4 text-xs font-bold uppercase tracking-wider transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#168a7a] focus-visible:ring-offset-2 focus-visible:ring-offset-white active:scale-[0.98]";
const UPLOAD_BTN_CLASS =
  "mt-3 inline-flex min-h-[44px] cursor-pointer items-center gap-2 rounded-full border-2 px-4 text-[11px] font-black uppercase tracking-[0.14em] transition-colors hover:bg-[#0a3d3a0d] focus-within:ring-2 focus-within:ring-[#168a7a] focus-within:ring-offset-2";
    ...
    <main className="font-editorial" style={{ background: C.cream, color: C.ink }}>
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <header className="mb-6">
          ...
          <h1 className="mt-1 font-display text-[28px] leading-tight sm:text-4xl">
            Bliv cleaner hos MyCleaner
          </h1>
          <p className="mt-2 max-w-xl text-sm leading-relaxed opacity-75">
            Vi guider dig trin for trin. Alt gemmes automatisk, så du kan holde pause og fortsætte senere.
          </p>
          <div
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs opacity-75">
            <span className="font-bold">{overallPct}% færdig</span>
            <span aria-hidden="true">·</span>
            <span>Status: {pp.status.replace(/_/g, " ")}</span>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <button
                ...
                onClick={() => setStep(i)}
                type="button"
                className="flex min-h-[64px] min-w-[92px] snap-start flex-col items-center justify-center gap-1.5 rounded-xl border-2 p-2 text-[10px] font-bold uppercase tracking-wider transition-all hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#168a7a] focus-visible:ring-offset-2 sm:min-w-0"
                style={{
                ...
                aria-label={`Trin ${i + 1} af ${STEPS.length}: ${s.title}${done ? " — færdig" : " — mangler"}`}
              >
        ...
        <section className="rounded-2xl border-2 bg-white p-5 shadow-[0_20px_44px_-32px_rgba(10,61,58,0.45)] sm:p-7" style={{ borderColor: `${C.ink}22` }}>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
          <button
            ...
            disabled={step === 0}
            className="inline-flex min-h-[44px] items-center gap-2 rounded-full border-2 px-5 text-[11px] font-bold uppercase tracking-[0.18em] transition-colors hover:bg-[#0a3d3a0d] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#168a7a] focus-visible:ring-offset-2 disabled:opacity-35"
            style={{ borderColor: C.ink, color: C.ink }}
          >
            <ArrowLeft className="h-3.5 w-3.5" aria-hidden="true" /> Forrige
          </button>
          {step < STEPS.length - 1 && (
            <button
              type="button"
              onClick={() => setStep((s) => Math.min(STEPS.length - 1, s + 1))}
              className="inline-flex min-h-[44px] flex-1 items-center justify-center gap-2 rounded-full px-6 text-[11px] font-bold uppercase tracking-[0.18em] shadow-[4px_4px_0_rgba(10,61,58,0.18)] transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a3d3a] focus-visible:ring-offset-2 active:translate-y-0 sm:flex-none"
              style={{ background: currentComplete ? C.teal : C.orange, color: C.cream }}
            >
              <span className="truncate">Næste: {STEPS[step + 1].title}</span>
...
function StepAccount({ user, authProfile }: { user: any; authProfile: any }) {
  return (
    <div className="space-y-4">
      <h2 className="font-display text-2xl">Velkommen ombord</h2>
      ...
      <div className="rounded-xl border-2 p-4 text-xs leading-relaxed" style={{ borderColor: `${C.ink}33`, background: "#ffffff" }}>
      ...
      <div className="rounded-xl p-4 text-sm leading-relaxed" style={{ background: C.cream }}>
...
function StepBasic({
  ...
  return (
    <div className="space-y-4">
      ...
      <Field label="Visningsnavn (offentligt)" hint="Sådan vises du for kunder — fx “Maria K.”">
        <input
          className={INPUT_CLASS}
      ...
      <Field label="Dit arbejdsområde" hint="Vi viser kun by og postnummer — aldrig din præcise adresse.">
        <PostalCodeCityField
          countryCode={(pp.base_country_code || authProfile?.country_code || "DK").toUpperCase()}
      ...
      <Field label="Profilfoto">
        <div className="flex flex-col gap-4 rounded-2xl border-2 p-4 sm:flex-row sm:items-center" style={{ borderColor: `${C.ink}33`, background: C.cream }}>
          ...
          <div className="flex-1">
            ...
            <label className={UPLOAD_BTN_CLASS} style={{ borderColor: C.ink }}>
...
function StepService({
  ...
  return (
    <div className="space-y-4">
      ...
      <FieldGroup label="Kategorier" hint="Vælg de opgavetyper, du gerne vil løse.">
      ...
      <Field label="Overskrift" hint="Én linje der fanger kunden.">
        <input
          className={INPUT_CLASS}
          value={pp.headline || ""}
      ...
      <Field label="Beskrivelse (mindst 40 tegn)">
        <textarea
          className={`${INPUT_CLASS} min-h-[132px] leading-relaxed`}
          value={pp.bio || ""}
      ...
      <FieldGroup label="Sprog" hint="Kunder ser, hvilke sprog du taler.">
        <div className="flex flex-wrap gap-2">
          {LANGUAGES.map((l) => {
...
function StepInsurance({ pp, patch }: { pp: ProviderProfile; patch: (u: Partial<ProviderProfile>) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Field label="Forsikringen gælder til" hint="Vi minder dig, inden policen udløber.">
        <input
          type="date"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-4 rounded-xl border-2 p-4" style={{ borderColor: `${C.ink}22` }}>
        <label className="flex min-h-[44px] cursor-pointer items-start gap-3 text-sm leading-relaxed focus-within:ring-2 focus-within:ring-[#168a7a] focus-within:ring-offset-4 focus-within:ring-offset-white">
          <input
            type="checkbox"
            className="mt-0.5 h-[20px] w-[20px] shrink-0 cursor-pointer accent-[#168a7a]"
    ...
    <div className="space-y-4">
      ...
      {submitted ? (
        ...
        <button
          ...
          className="inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-full px-6 text-xs font-bold uppercase tracking-[0.2em] shadow-[4px_4px_0_rgba(10,61,58,0.2)] transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0a3d3a] focus-visible:ring-offset-2 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0 sm:w-auto"
        ...
        >
          ...
          {submitting ? "Indsender…" : null}
          {submitting ? null : "Indsend til godkendelse"}
        </button>
      )}
      {!canSubmit && !submitted && (
...
function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-[11px] font-black uppercase tracking-[0.14em] opacity-80">{label}</span>
      {hint && <span className="mb-2 block text-xs leading-relaxed opacity-70">{hint}</span>}
      {children}
    </label>
  );
}

/** Grouped controls (chips/toggles) — a fieldset keeps screen readers accurate. */
function FieldGroup({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <fieldset className="block border-0 p-0">
      <legend className="mb-1.5 block text-[11px] font-black uppercase tracking-[0.14em] opacity-80">{label}</legend>
      {hint && <p className="mb-2 text-xs leading-relaxed opacity-70">{hint}</p>}
      {children}
    </fieldset>
  );
}
...
function StatusRow({ ok, label, hint }: { ok: boolean; label: string; hint?: string }) {
  return (
    <div
      className="flex items-start gap-3 rounded-xl border-2 p-3.5"
      style={{ borderColor: ok ? `${C.teal}55` : `${C.ink}22`, background: ok ? "#168a7a0f" : "transparent" }}
    >
      {ok ? (
      ...
      <div>
        ...
        {hint && <div className="mt-0.5 text-xs leading-relaxed opacity-75">{hint}</div>}
      </div>
    </div>
  );
}

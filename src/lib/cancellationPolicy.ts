 ...
 * explicit, separately approved decision — several legal documents quote them
 * verbatim.
 *
 * NOT RETROACTIVE. Every published ladder keeps living in
 * `CANCELLATION_POLICY_VERSIONS`. A booking is always evaluated with the
 * version stored in its `cancellation_policy_snapshot`; a booking without a
 * snapshot falls back to the legacy version, never to the newest one.
 *
 * ACTIVATION. v2 (18/8) only becomes the policy for NEW bookings at
 * `CANCELLATION_POLICY_V2_ACTIVATION_AT` AND only while the kill switch below
 * is on. Fail-safe is always v1.0.0 (48/24).
 */

/**
 * Kill switch — frontend mirror. The AUTHORITATIVE switch is the edge-function
 * copy, which reads the server-only env var `CANCELLATION_POLICY_V2_ENABLED`.
 * This value is compiled into the bundle and can therefore never be changed by
 * a client at runtime; flipping it off requires a redeploy. Money outcomes are
 * decided exclusively by the backend copy.
 */
const CANCELLATION_POLICY_V2_ENABLED_DEFAULT = true;

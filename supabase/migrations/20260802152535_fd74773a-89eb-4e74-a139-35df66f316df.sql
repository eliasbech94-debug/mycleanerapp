...
BEGIN
  ...
  -- Calendar availability revalidation (working hours + manual blocks).
  -- Providers without configured working hours are unaffected (backwards compatible).
  v_avail := public.validate_provider_slot_v1(
    v_uid, v_starts, GREATEST(1, (EXTRACT(epoch FROM (v_ends - v_starts)) / 60)::int));
  IF (v_avail->>'code') IN ('OUTSIDE_WORKING_HOURS', 'CALENDAR_SLOT_UNAVAILABLE') THEN
    RETURN jsonb_build_object('status','slot_conflict','reason', v_avail->>'code');
  END IF;

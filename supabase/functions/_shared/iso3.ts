// ISO 3166-1 alpha-2 -> alpha-3 country codes (full official list, 249 entries).
//
// Sumsub's applicant API rejects alpha-2 codes ("Country 'DK' is not valid.
// Use alpha-3 code."), while MyCleaner stores alpha-2 everywhere
// (profiles.country_code, market config, addresses). This is the single
...
/**
 * Convert an ISO 3166-1 alpha-2 country code to alpha-3.
 * Returns null for unknown/blank input so callers can omit the field rather
 * than send something the provider will reject.
 */
export function toAlpha3(code: string | null | undefined): string | null {
  if (!code) return null;
  const key = code.trim().toUpperCase();
  if (key.length === 3) {
    // Already alpha-3: only accept it if it is a real code.
    return Object.values(ALPHA2_TO_ALPHA3).includes(key) ? key : null;
  }
  return ALPHA2_TO_ALPHA3[key] ?? null;
}

export { ALPHA2_TO_ALPHA3 };

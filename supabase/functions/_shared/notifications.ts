...
export async function runHealthCheck(
  ...
  if (!profile?.phone || !PHONE_RE.test(profile.phone)) {
    drafts.push({
      ...
      action_label: "Ret telefonnummer", action_url: "/profil?tab=info",
  ...
  if (!addresses || addresses.length === 0) {
    ...
    if (noPrimary) {
      drafts.push({
        ...
        action_label: "Vælg primær", action_url: "/profil?tab=addresses",
    ...
    if (missingAccess.length > 0) {
      drafts.push({
        ...
        action_label: "Udfyld adgangs-info", action_url: "/profil?tab=addresses",
  ...
  for (const b of upcoming ?? []) {
    drafts.push({
      ...
      action_label: "Se booking", action_url: "/profil?tab=bookings",

...
const users: Seed[] = [
  { email: "test.customer@mycleaner.dev", password: "TestPass!2026", role: "customer", full_name: "Test Customer", country: "DK" },
  { email: "test.provider@mycleaner.dev", password: "TestPass!2026", role: "provider", full_name: "Test Provider", country: "DK", providerSlug: "mette-copenhagen" },
  { email: "test.support@mycleaner.dev",  password: "TestPass!2026", role: "support",  full_name: "Test Support",  country: "DK" },
  { email: "test.admin@mycleaner.dev",    password: "TestPass!2026", role: "admin",    full_name: "Test Admin",    country: "DK" },
];
...
async function upsertUser(seed: Seed) {
  ...
  if (!id) {
    ...
    await admin.auth.admin.updateUserById(id, { password: seed.password, email_confirm: true });
  ...
  if (seed.role !== "customer") {
    await admin.from("user_roles").upsert({ user_id: id, role: seed.role }, { onConflict: "user_id,role" });
  }
...
(async () => {
  ...
  for (const u of users) {
    try { await upsertUser(u); } catch (e) { console.error(`  ✗ ${u.email}:`, e); process.exitCode = 1; }
  }
  console.log("Done.");
})();
